---
applyTo: '**'
description: 'Effective Dart and Riverpod coding guidelines for GitHub Copilot completions and PR review suggestions.'
---

# Github Copilot Custom Instructions for Effective Dart
The following follows the [.github/copilot-instructions.md](https://docs.github.com/en/copilot/how-tos/custom-instructions) format.

## When reviewing pull requests
Copilot should use these rules to make inline PR comments or suggestions during code review, especially for style, architecture, and provider usage.

- Flag style or naming issues violating Effective Dart.
- Flag providers created inside widgets or classes.
- Highlight missing doc comments on public APIs.
- Check for unnecessary rebuilds, missing const constructors.
- Suggest replacing Ephemeral state providers with local widget state.

These rules apply to both Copilot PR comment suggestions and manual code reviews.

## When assisting with code completion
- Follow these guidelines when suggesting or completing Dart or Riverpod code.
- Prefer concise, idiomatic Flutter patterns aligned with Effective Dart.
- Avoid suggesting anti-patterns such as mutable providers, redundant async keywords, or unnecessary rebuilds.
- Prefer minimal, compile-safe completions that align with Riverpod lint rules and Flutter idioms.
...
Always use [Riverpod](https://pub.dev/packages/riverpod) providers wherever possible, and for state management, as well as user friendly tests.
Follow the [DO and DON'T guidelines for using Riverpod](https://riverpod.dev/docs/essentials/do_donts).
...
Providers should manage shared, app-wide state. Avoid using them for ephemeral UI state (selection, form input, animation controllers, etc.) that should reset when leaving a route.

- The currently selected item.
- Form state. Because leaving and re-entering the form should typically reset the form state. This includes pressing the back button during a multi-page form.  
- Animations.  
...
#### Prefer using AsyncValue.when or map for readability
Avoid manually checking for AsyncValue states.  
