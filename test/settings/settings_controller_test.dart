...
void main() {
  ...
  group("SettingsController", () {
    ...
    test(
      "setLevel should clamp level to 1 if value is less than 1 and persist it",
      () async {
        when(() => mockRepo.readInt(SharedPreferencesKey.level)).thenReturn(1);
        when(() => mockRepo.writeInt(any(), any())).thenAnswer((_) async {});
        ...
        verify(
          () => mockRepo.writeInt(SharedPreferencesKey.level, 1),
        ).called(1);
      },
    );

    test(
      "setLevel should clamp level to 150 if value is greater than 150 and persist it",
      () async {
        when(() => mockRepo.readInt(SharedPreferencesKey.level)).thenReturn(1);
        when(() => mockRepo.writeInt(any(), any())).thenAnswer((_) async {});
        ...
        verify(
          () => mockRepo.writeInt(SharedPreferencesKey.level, 150),
        ).called(1);
      },
    );
  });
